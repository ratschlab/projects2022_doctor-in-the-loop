...
##Unbalanced indices
idx = np.zeros(shape=(1000,10), dtype=int)
labels= cifar10_100epochs.y
for c in np.unique(labels):
    idx[:, c]= np.where(labels==c)[0]
np.random.shuffle(idx)
n_train=[800, 80, 40, 800, 800, 40, 80, 800, 800, 40]
n_test= [200, 20, 10, 200, 200, 10, 20, 200, 200, 10]
idx_train_unbalanced= np.array([], dtype=int)
idx_test_unbalanced= np.array([], dtype=int)
for c in np.unique(labels):
    idx_train_unbalanced= np.concatenate((idx_train_unbalanced,idx[:n_train[c], c]), axis=0)
    idx_test_unbalanced= np.concatenate((idx_test_unbalanced, idx[n_train[c]:n_test[c]+n_train[c], c]), axis=0)
