    ...
    def query(self, M, B=1, plot=[False, False], n_initial=1):
        ...
        while count < M:
            ...
            if show_all_clusters:
                fig, ax = plt.subplots()
                ax.axis('equal')
                sns.scatterplot(data=self.dataset, x="x1", y="x2", hue="cluster_id",
                                palette=sns.color_palette("husl", B + n_labeled), ax=ax)
            ...
            if show_all_clusters or show_clusters:
                sns.scatterplot(data=self.dataset[self.dataset.queries], x="x1", y="x2", color="black", marker="P",
                                s=100,
            ...
            if show_all_clusters or show_clusters:
                sns.scatterplot(data=self.dataset.iloc[self.dataset.queries[count - B:count]], x="x1", y="x2", s=100,
                                color="red", marker="P")
...
class ProbCoverSampler_Faiss(ActiveLearner):
    ...
    def query(self, M, B=1, n_initial=1):
        ...
        for _ in range(M):
            ...
            #max_out_degree = np.argmax(out_degrees*(1-self.dataset.labeled == 0))
    ...
    def adaptive_query(self, M, K=3, B=1, n_initial=1):
        ...
        if len(self.dataset.queries) > 0:
            for l in self.dataset.queries:
                ...
                n_covered = np.array(
                    [np.sum((lims[l - 1] <= covered_by_l_I_id) & (covered_by_l_I_id < lims[l])) for l in
                     range(1, len(lims))])
        ...
        for m in range(M):
            ...
            if len(self.dataset.queries) > 0:
                ...
                D_neighbours, I_neighbours = index_knn.search(self.dataset.x.astype("float32"),
                                                              n_neighbours)  # find K-nn for all
                ...
                new_radiuses[self.dataset.labeled] = self.dataset.radiuses[
                    self.dataset.labeled]  # insure radius of labeled points are not changed by the kmeans averaging
            ...
            if len(self.dataset.queries) > 0:
                ...
                diff_radiuses, new_radiuses = np.zeros(shape=self.dataset.radiuses.shape), np.zeros(
                    shape=self.dataset.radiuses.shape)
                diff_radiuses[self.dataset.queries] = self.dataset.radiuses[self.dataset.queries] + rc - d
                new_radiuses[self.dataset.queries] = np.maximum(0, 0.5 * (
                            self.dataset.radiuses[self.dataset.queries] - rc + d))
                mask = (diff_radiuses > 0) * (self.dataset.y[c_id] != self.dataset.y) * (self.dataset.labeled == 1)
                new_radiuses[np.invert(mask)] = self.dataset.radiuses[
                    np.invert(mask)]  # insure the radius remains unchanged for other points
                if mask.any():
    ...
    while len(dataset.queries) < M:
        if ((not covered) & (len(dataset.queries) + B <= M)):
            ...
            while covered:
                ...
                if changed:
                    radius = get_radius(purity_threshold, dataset.x, pseudo_labels, search_range=[0, 10],
                                        search_step=0.01)
                    print(
                        f"The labels were changed with {100 * accuracy_clustering(old_labels, pseudo_labels)}% overlap and the new radius is {radius}")
                else:
                    purity_threshold += (1 - purity_threshold) / 2
                    radius = get_radius(purity_threshold, dataset.x, pseudo_labels, search_range=[0, 10],
                                        search_step=0.01)
                    print(
                        f"The labels were not changed so we set a higher purity threshold {purity_threshold} with radius {radius}")
...
class ProbCoverSampler(ActiveLearner):
    def __init__(self, dataset, purity_threshold,
        ...
        self.radius = get_radius(self.purity_threshold, self.dataset.x, self.pseudo_labels, search_range, search_step,
                                 plot_unpure_balls)
        print(
            f"ProbCover Sampler initialized for threshold {self.purity_threshold} with radius {round(self.radius, 2)}")
    ...
    def adaptive_query(self, M, K=3, B=1, n_initial=1):
        ...
        for m in range(M):
            ...
            max_out_degree = np.argmax(out_degrees[self.dataset.labeled == 0])
            c_id = np.arange(n_pool)[self.dataset.labeled == 0][max_out_degree]
            assert ((out_degrees[c_id] == np.max(out_degrees[self.dataset.labeled == 0])) & (
                        self.dataset.labeled[c_id] == 0))
            rc = self.dataset.radiuses[c_id]
