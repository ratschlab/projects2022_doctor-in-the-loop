...
from helper import get_radius_faiss, adjacency_graph_faiss, remove_incoming_edges_faiss, update_adjacency_radius_faiss, reduce_intersected_balls_faiss
import faiss
    ...
    def adaptive_query(self, M, K=3, B=1, n_initial=1):
        ...
        for m in range(M):
            ...
            if len(self.dataset.queries)>0:
                ...
                D_neighbours, I_neighbours = index_knn.search(self.dataset.x[self.dataset.labeled==0].astype("float32"), n_neighbours)  # find K-nn for all
                ...
                new_radiuses[self.dataset.labeled==0]= alpha*(weights*self.dataset.radiuses[self.dataset.queries[I_neighbours]]).sum(axis=1)+(1-alpha)*self.dataset.radiuses[self.dataset.labeled==0]
                print(self.lims[-1], self.D.shape, self.I.shape)

                self.lims, self.D, self.I= update_adjacency_radius_faiss(self.dataset, new_radiuses, self.lims_ref, self.D_ref, self.I_ref, self.lims, self.D, self.I)
                print(self.lims[-1], self.D.shape, self.I.shape)
            # get the unlabeled point with highest out-degree
            out_degrees = self.lims[1:] - self.lims[:-1]
            if np.any(out_degrees > 0):
            ...
            self.lims, self.D, self.I= reduce_intersected_balls_faiss(self.dataset, c_id, self.lims_ref, self.D_ref, self.I_ref, self.lims, self.D, self.I)
